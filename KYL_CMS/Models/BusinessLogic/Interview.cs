...
{
    public class Interview : DatabaseAccess
    {
        public Interview(string connectionStringName) : base(connectionStringName)
        { }
...
";
        ...
        public CaseRetrieveRes PaginationRetrieve(CaseRetrieveReq req, string UserId)
        {
...
SELECT TOP(@TOP) SN,NAME1,NAME2,NAME3,NAME4,NAME5,NAME6,TEL1,TEL2,TEL3,TEL4,TEL5,TEL6,YEARS_OLD,ADDRESS,TEACHER1,TEACHER2,TEACHER3,FAMILY,FAMILY_FILE,RESIDENCE,NOTE,PROBLEM,EXPERIENCE,SUGGEST
    FROM CASE_OWNER{0}{1};";
                string where = "";
                ...
                where += " AND SN IN(SELECT DISTINCT CASE_SN FROM INTERVIEW WHERE VOLUNTEER_SN = @VOLUNTEER_SN AND CASE_STATUS IN ('01','03') )";
        ...
        public bool CheckPharseUsed(PhraseModifyReq req)
        {
            bool used = false;
            ...
            using (DbCommand cmd = Db.CreateConnection().CreateCommand())
            {
                switch (req.PHRASE.PHRASE_GROUP)
                {
                    case "CONTACT_TIME":
                    case "TREATMENT":
                    case "CASE_SOURCE":
                    ...
                    case "VISITED":
                    case "SOLVE_DEGREE":
                        sql = "SELECT TOP 1 'Y' AS USED FROM INTERVIEW WHERE "+ req.PHRASE.PHRASE_GROUP  + "= @"+ req.PHRASE.PHRASE_GROUP;
                        Db.AddInParameter(cmd, req.PHRASE.PHRASE_GROUP, DbType.String, req.PHRASE.PHRASE_KEY);
                        break;
                    case "SPECIAL_IDENTITY":
                        sql = "SELECT TOP 1 'Y' AS USED FROM SPECIAL_IDENTITY WHERE PHRASE_KEY=@PHRASE_KEY ";
                        Db.AddInParameter(cmd, "PHRASE_KEY", DbType.String, req.PHRASE.PHRASE_KEY);
                        break;
                    case "INTERVIEW_CLASSIFY":
                        sql = "SELECT TOP 1 'Y' AS USED FROM INTERVIEW_CLASSIFY WHERE PHRASE_KEY=@PHRASE_KEY ";
                        Db.AddInParameter(cmd, "PHRASE_KEY", DbType.String, req.PHRASE.PHRASE_KEY);
                        break;
                    case "FEELING":
                        sql = "SELECT TOP 1 'Y' AS USED FROM FEELING WHERE PHRASE_KEY=@PHRASE_KEY ";
                        Db.AddInParameter(cmd, "PHRASE_KEY", DbType.String, req.PHRASE.PHRASE_KEY);
                        break;
                    case "INTERVENTION":
                        sql = "SELECT TOP 1 'Y' AS USED FROM INTERVENTION WHERE PHRASE_KEY=@PHRASE_KEY ";
                        Db.AddInParameter(cmd, "PHRASE_KEY", DbType.String, req.PHRASE.PHRASE_KEY);
                        break;
                }
        ...
        public bool CheckUserIdUsed(string UserId)
        {
            bool used = false;
            ...
            using (DbCommand cmd = Db.CreateConnection().CreateCommand())
            {
                sql = "SELECT TOP 1 'Y' AS USED FROM INTERVIEW WHERE VOLUNTEER_SN=@VOLUNTEER_SN ";
                Db.AddInParameter(cmd, "VOLUNTEER_SN", DbType.String, UserId);
