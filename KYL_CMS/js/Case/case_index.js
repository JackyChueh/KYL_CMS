var CaseIndex = {
    ...
    Page_Init: function () {
        CaseIndex.EventBinding();
        CaseIndex.OptionRetrieve();
        CaseIndex.ActionSwitch('R');
        //CaseIndex.DataValidate();
        ...
        //    if (CaseIndex.Action != "R") {
        //        return false;
        //    }
        //    else {
        //        if (CaseIndex.Action == "U") {
        //            if (CaseIndex.DataChanged()) {
        //                return false;
        //            }
        //        }
        //    }
    ...
    EventBinding: function () {
        ...
        $('#create').click(function () {
            CaseIndex.ActionSwitch('C');
        });

        $('#save').click(function () {
            if (CaseIndex.DataValidate()) {
                if (CaseIndex.Action === 'C') {
                    CaseIndex.CaseCreate();
                } else if (CaseIndex.Action === 'U') {
                    CaseIndex.CaseUpdate();
                }
            }
        });

        $('#undo').click(function () {
            CaseIndex.ValueRecover(CaseIndex.Action);
        });
        ...
        $("#file1").change(function () {
            ...
            if (document.getElementById("file1").files[0]) {
                var url = CaseIndex.FileUrl(document.getElementById("file1").files[0]);
                master_sectoin.find('[name=FAMILY_FILE]').attr('src', url);
            }
            else {
        ...
        $('#add').click(function () {
            ...
            var input = template.find('input[name="INCOMING_DATE"]');
            var d = new Date();
            $(input).val(d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate() + ' ' + d.getHours() + ':' + d.getMinutes());
            CaseIndex.DateRangePickerBind(input);
        ...
        $('#modal #confirm').click(function () {
            $('#modal #confirm').hide();
            $('#modal').modal('hide');
            CaseIndex.CaseDelete();
        });

        $('#detail_save').click(function () {
            CaseIndex.DetailSave(false);
        });
        $('#detail_assign').click(function () {
            CaseIndex.DetailSave(true);
        });


        $('#detail_delete').click(function () {
            CaseIndex.DetailDelete();
        });

        var section_retrieve = $('#section_retrieve');
        var obj = null;
        obj = section_retrieve.find('[name=DATE_RANGE]');
        CaseIndex.DateRangePickerBind2(obj);

        obj.change(function () {
            if (!obj.val()) {
                CaseIndex.QueryStartDate = null;
                CaseIndex.QueryEndDate = null;
            }
        });
    ...
    CaseRetrieve: function () {
        ...
        var request = {
            ...
            CASE: {
                ...
                START_DATE: CaseIndex.QueryStartDate,
                END_DATE: CaseIndex.QueryEndDate,
                ...
                if (response.ReturnStatus.Code === 0) {
                    ...
                    if (response.Pagination.RowCount > 0) {
                        $.each(response.CASE, function (idx, row) {
                            htmlRow = '<tr>';
                            htmlRow += '<td><a class="fa fa-edit fa-lg" onclick="CaseIndex.RowSelected(' + row.SN + ');" data-toggle="tooltip" data-placement="right" title="修改" style="cursor: pointer;"></a></td>';
    ...
    CaseCreate: function () {
        var url = '/Case/CaseCreate';

        var master_sectoin = $('#master_sectoin');
        var master = {
            UserId: CaseIndex.UserId,
            CASE: {
                NAME1: master_sectoin.find('[name=NAME1]').val(),
                NAME2: master_sectoin.find('[name=NAME2]').val(),
                NAME3: master_sectoin.find('[name=NAME3]').val(),
                NAME4: master_sectoin.find('[name=NAME4]').val(),
                NAME5: master_sectoin.find('[name=NAME5]').val(),
                NAME6: master_sectoin.find('[name=NAME6]').val(),
                TEL1: master_sectoin.find('[name=TEL1]').val(),
                TEL2: master_sectoin.find('[name=TEL2]').val(),
                TEL3: master_sectoin.find('[name=TEL3]').val(),
                TEL4: master_sectoin.find('[name=TEL4]').val(),
                TEL5: master_sectoin.find('[name=TEL5]').val(),
                TEL6: master_sectoin.find('[name=TEL6]').val(),
                YEARS_OLD: master_sectoin.find('[name=YEARS_OLD]').val(),
                ADDRESS: master_sectoin.find('[name=ADDRESS]').val(),
                TEACHER1: master_sectoin.find('[name=TEACHER1]').val(),
                TEACHER2: master_sectoin.find('[name=TEACHER2]').val(),
                TEACHER3: master_sectoin.find('[name=TEACHER3]').val(),
                FAMILY: master_sectoin.find('[name=FAMILY]').val(),
                RESIDENCE: master_sectoin.find('[name=RESIDENCE]').val(),
                NOTE: master_sectoin.find('[name=NOTE]').val(),
                PROBLEM: master_sectoin.find('[name=PROBLEM]').val(),
                EXPERIENCE: master_sectoin.find('[name=EXPERIENCE]').val(),
                SUGGEST: master_sectoin.find('[name=SUGGEST]').val(),
                MERGE_REASON: master_sectoin.find('[name=MERGE_REASON]').val(),
                CASE_DETAIL: CaseIndex.INTERVIEW
            }
        };
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (response.ReturnStatus.Code === 1) {
                    CaseIndex.CaseQuery(response.CASE.SN);
                }
                $('#modal .modal-title').text('交易訊息');
            ...
            success: function (data) {
                ...
                if (response.ReturnStatus.Code === 0) {
                    if (response.CASE) {
                        ...
                        CaseIndex.INTERVIEW = response.CASE.CASE_DETAIL;
                        CaseIndex.DetailRefresh();
                        CaseIndex.CASE = response.CASE;
                        //$('#section_modify').validate();
                    }
                    CaseIndex.ActionSwitch('U');
                }
                else {
                    $('#modal .modal-title').text('交易訊息');
                    $('#modal .modal-body').html('<p>交易代碼:' + response.ReturnStatus.Code + '<br/>交易說明:' + response.ReturnStatus.Message + '</p>');
    ...
    DetailRefresh: function () {
        if (CaseIndex.INTERVIEW.length > 0) {
    ...
    DetailSelected: function (idx) {
        ...
        var value = CaseIndex.INTERVIEW[idx];
        ...
        CaseIndex.DateRangePickerBind(template.find('input[name=INCOMING_DATE]'));
    ...
    DetailValidate: function () {
        ...
        if (detail_interview) {
            ...
            var treatment = template.find('select[name=TREATMENT]');
            if ('04,05,06'.indexOf(treatment.val()) < 0) {
    ...
    AssignValidate: function () {
        ...
        if (detail_interview) {
            ...
            if (template.find('input[name=DURING]').val()) {
                var bool = /^\d+$/.test(template.find('input[name=DURING]').val());
                if (!bool) {
                    ErrMsg += '談話時間輸入錯誤<br/>';
                }
                    ...
                    var during = parseInt(template.find('input[name=DURING]').val());
                    if (during < 0 || during > 1440) {
                        ErrMsg += '談話時間必需介於0~1440<br/>';
                    }
                }
            }
            else {
                template.find('input[name=DURING]').val(0);
            }
            if (ErrMsg) {
                //$('#detail_modal').animate({
                //    scrollTop: 0
    ...
    DetailSave: function (isAssign) {

        if (isAssign) {
            if (!CaseIndex.AssignValidate()) {
                return false;
            }
        }
        else {
            if (!CaseIndex.DetailValidate()) {
                return false;
            }
        }
        ...
        if (detail_section.data('row') === 'add') {
            CaseIndex.INTERVIEW.unshift(detail);
        }
        else if (detail_section.data('row') === 'edit') {
            CaseIndex.INTERVIEW[detail_section.data('index')] = detail;
        }
        //console.log(CaseIndex.INTERVIEW)
    ...
    DetailDelete: function () {
        var detail_section = $('#detail_interview');
        CaseIndex.INTERVIEW[detail_section.data('index')].DataRow = 'delete';
        //console.log(CaseIndex.INTERVIEW);
        $('#detail_modal').modal('hide');
        CaseIndex.DetailRefresh();
    ...
    ValueRecover: function (action) {
        ...
        if (action === 'U') {
            //var section_modify = $("#section_modify");
            CaseIndex.CaseQuery(section_modify.find('[name=SN]').val());
        }
        else {
            //var section_modify = $("#section_modify");
    ...
    DateRangePickerBind2: function (obj) {
        ...
        obj.daterangepicker({
            ...
            CaseIndex.QueryStartDate = start;
            CaseIndex.QueryEndDate = end;
